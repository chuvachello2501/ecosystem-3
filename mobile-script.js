document.addEventListener('DOMContentLoaded', () => {
  ...
  const sectors = document.querySelectorAll('.ecs3-sector-hex');
  const cards = document.querySelectorAll('.ecs3-card');

  // ===== ПОДСВЕТКА КАРТОЧКИ =====
  function highlightCard(category, add) {
    cards.forEach(card => {
      if (card.dataset.category === category) {
        card.classList.toggle('ecs3-highlighted', add);
      }
    });
  }
  ...
  sectors.forEach(sector => {
    ...
    sector.addEventListener('click', (e) => {
      e.stopPropagation();
      const targetCard = document.querySelector(`.ecs3-card[data-category="${category}"]`);
      if (targetCard) {
        ...
        if (window.innerWidth < 1440) {
          ...
          const btn = document.querySelector('.ecs3-floating-hub-btn');
          if (btn) btn.classList.remove('ecs3-visible');
        }
      }
    });
  });
  ...
  btn.className = 'ecs3-floating-hub-btn';
btn.innerHTML = `<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  document.querySelector(".ecs3-ecosystem-standalone-project").appendChild(btn);

  // Клик по кнопке → возврат наверх
  btn.addEventListener('click', () => {
    window.scrollTo({ top: 0, behavior: 'smooth' });
    btn.classList.remove('ecs3-visible');
  });
  ...
  function updateButtonVisibility(isHubVisible) {
    ...
    if (isHubVisible) {
      ...
      btn.classList.add('ecs3-visible');
    }
  }

  // IntersectionObserver для отслеживания видимости диаграммы
  const centralHub = document.querySelector('.ecs3-central-hub');
  if (centralHub) {
  ...
  document.addEventListener('click', (e) => {
    if (!e.target.closest('.sector-hub') && !e.target.closest('.ecs3-floating-hub-btn')) {
      document.querySelectorAll('.ecs3-card.ecs3-highlighted, .ecs3-sector-hex.ecs3-active')
        .forEach(el => el.classList.remove('ecs3-highlighted', 'ecs3-active'));
    }
  });
});
...
(function() {
  function replaceBrokenImages() {
    document.querySelectorAll('img.ecs3-tag').forEach(img => {
